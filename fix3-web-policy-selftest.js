...
function Hz(e=``){let t=String(e).toLowerCase(),n=/继续.*直到.*完成|直到.*完成|彻底完成|until.*complete|until.*done|finish.*everything/.test(t)?36:/代码|文件|项目|修复|汉化|仓库|构建|测试|code|file|project|fix|repo|build|test/.test(t)?24:12;return{maxSteps:n,maxNudges:8,stepTimeoutMs:NR,requestDelayMinMs:PR,requestDelayMaxMs:FR,fullToolResultWindow:4}}
...
ok('step-default',Hz('hello').maxSteps===12);
ok('step-project',Hz('修复这个项目').maxSteps===24);
ok('step-until-done',Hz('继续直到完成').maxSteps===36);
