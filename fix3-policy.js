(()=>{
  ...
  function routeBonus(desc,intent){if(!flags.adaptiveRouting)return 0;let n=baseName(desc),q=String(intent??``).toLowerCase(),b=0;let file=/(?:\u6587\u4ef6|\u76ee\u5f55|\u8bfb\u53d6|\u67e5\u770b|\u641c\u7d22|\u67e5\u627e|file|folder|directory|read|search|find|grep)/i.test(q),edit=/(?:\u4fee\u6539|\u7f16\u8f91|\u5199\u5165|\u66ff\u6362|\u8865\u4e01|edit|modify|write|patch|replace)/i.test(q),cmd=/(?:\u547d\u4ee4|\u6267\u884c|\u8fd0\u884c|\u7ec8\u7aef|\u6784\u5efa|\u6d4b\u8bd5|powershell|command|shell|run|exec|build|test)/i.test(q),git=/(?:git|github|\u4ed3\u5e93|\u63d0\u4ea4|\u5206\u652f|commit|branch|repo)/i.test(q);if(file&&/(read|list|find|search|grep|stat|snapshot)/.test(n))b+=650;if(edit&&/(patch|write|edit|replace)/.test(n))b+=800;if(edit&&/(read|find|search|grep)/.test(n))b+=260;if(cmd&&/(run_command|shell|exec|run)/.test(n))b+=900;if(edit&&cmd&&/(run_command|shell|exec|run)/.test(n))b+=400;if(git&&/(git|run_command|shell|exec)/.test(n))b+=620;return b}
  ...
  function stepLimit(text){if(!flags.dynamicStepLimit)return 20;let q=String(text??``).toLowerCase();if(/^\s*(?:\u7ee7\u7eed|\u63a5\u7740|\u7ee7\u7eed\u5427|\u7ee7\u7eed\u6267\u884c|continue|go\s+on|carry\s+on)\s*[\u3002.!\uff01\uff1f?]*\s*$/i.test(q)||/(?:\u7ee7\u7eed.*(?:\u76f4\u5230|\u5230).*\u5b8c\u6210|\u76f4\u5230.*\u5b8c\u6210|\u5f7b\u5e95\u5b8c\u6210|until.*complete|until.*done|finish.*everything)/i.test(q))return 36;if(/(?:\u4ee3\u7801|\u6587\u4ef6|\u9879\u76ee|\u4fee\u590d|\u6c49\u5316|\u4ed3\u5e93|\u6784\u5efa|\u6d4b\u8bd5|code|file|project|fix|repo|build|test)/i.test(q))return 24;return 12}
  ...
  g.DPP_FIX3=Object.freeze({flags,compactResult,modelBudget,classifyError,shouldRetry,retryDelay,noteRetry,routeBonus,recordSuccess,recentBonus,stepLimit,newLoopState,beforeCall,blockedResult,longCommandInfo,healthFromCache,status});
})();