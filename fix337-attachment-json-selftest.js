...
const m=JSON.parse(fs.readFileSync(path.join(root,'manifest.json'),'utf8'));ok(['1.14.0.2','1.14.0.3','1.14.0.4'].includes(m.version),'real MV3 version bumped');ok(['1.14.0 ShunCode MCP Fix 3.3.7','1.14.0 ShunCode MCP Fix 3.3.8','1.14.0 ShunCode MCP Fix 3.3.9'].includes(m.version_name),'version name updated');
