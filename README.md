...
> Current release: **Fix 3.3.10.34** — investigation of the `.33` field report "send 继续, it runs for a moment, then stops again" (session `c0ba574e`, 23:12–23:18). All four loops ended `status=stopping / "已停止"`, and each stop timestamp matches a manual `mw_send_hook_seen` breadcrumb to the millisecond: the user typed a new message while the Agent was mid-step, and `DPP_ABORT_ACTIVE_AGENT_FOR_MANUAL_REQUEST_33107` abandoned the running loop by design. Not a regression — every step before the stop was a real HTTP-200 `run_command` execution and `run_command` was in the direct set. Two real defects were found alongside: (E) `DPP_NORMALIZE_RUN_COMMAND_RESULT_33108` regex-scanned the whole result including command output for `status=failed` / `exit_code=N`, so a `completed / exit_code 0` run that grepped the extension's own sources was reported as `run_command_failed` (3×); (F) a manual supersede only said "已停止" and recorded no diagnostic, so it read like a crash. `.34` (E) parses status/exit code from the `=== RUN_COMMAND BEGIN ===` header only (text before `--- OUTPUT BEGIN ---`, JSON-escape tolerant), falling back to the legacy full-text scan only when no marker exists; (F) `r1(reason)` accepts an explicit stop reason, the manual-request abort records turn_diag `manual_supersede_331034` and shows a localized "interrupted by your new message — wait for the status bar to finish, then send 继续" text. Ships `USAGE-zh_CN.md` (install, exposure mode, pinning, usage, recovery, evidence collection).

> Validation: dedicated Fix 3.3.10.34 suite **24/24 PASS** (includes the poisoned-output field case); all **55 self-test suites PASS** (`.33` baseline 54/54); hash-locked `.33 → .34` patcher (`tools/apply-fix331034.py`, 5-file SHA lock + 4 fail-closed anchors, refuses dst==src), re-apply fails closed, two independent rebuilds byte-identical (**208/208**). Manifest `1.14.0.39 / 1.14.0 ShunCode MCP Fix 3.3.10.34`. ZIP `DeepSeekPP-1.14.0-ShunCode-MCP-Fix3.3.10.34.zip` (13,946,628 B) SHA-256 `ECBCB43D8F07FD8961A7E0CDA676A59E1E1CC1B5C1F592CFB3AE6FB7A90329FA`.
