#!/usr/bin/env python3
"""Offline .40 release gate; never modifies live installs or uploads images.
python tools/validate-fix331040.py --root CANDIDATE --baseline BASE_36 --output NEW_EXTERNAL_DIR
.40 = .39 + skill-popup observe(null) startup-race fix in content-scripts/main-world.js.
"""
import argparse
import ast
import hashlib
...
def main():
    ...
    spec=importlib.util.spec_from_file_location('builder40',tools/'apply-fix331040.py');builder=importlib.util.module_from_spec(spec);spec.loader.exec_module(builder)
    ...
    check('version',mf['version']=='1.14.0.45' and mf['version_name']=='1.14.0 ShunCode MCP Fix 3.3.10.40')
    ...
    for lang in ['en','zh_CN']:
        d=json.loads((root/f'_locales/{lang}/messages.json').read_text(encoding='utf-8'));check('locale:'+lang,d['extension_name']['message']=='DeepSeek++ ShunCode MCP Fix 3.3.10.40')
    suites=sorted(root.glob('*selftest*.js'));check('55_legacy_suites',len(suites)==55)
    for n in ['apply-fix331037.py','apply-fix331038.py','apply-fix331039.py','apply-fix331040.py','validate-fix331040.py']:
    ...
    report={'release':'v1.14.0-fix3.3.10.40','passed':all(x['passed'] for x in checks+tests),'checks':checks,'tests':tests,'runtime_sha256':hashes,'browser_model_acceptance':'pending: reload + F5, extension error panel must stay free of main-world.js observe errors; slash-command skill popup still opens'}
