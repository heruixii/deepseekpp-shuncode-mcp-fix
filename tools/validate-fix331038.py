#!/usr/bin/env python3
"""Offline .38 release gate; never modifies live installs or uploads images.
python tools/validate-fix331038.py --root CANDIDATE --baseline BASE_36 --output NEW_EXTERNAL_DIR
.38 changes background.js: .37 diagnostics + DPP_REQUIRE_UPLOAD_CONTEXT_V7 no longer compares the stale sender.url conversation id
against the browser-owned tab conversation id. Every other gate function must be byte-identical to .36.
"""
import argparse
import ast
import hashlib
...
def main():
    ...
    spec=importlib.util.spec_from_file_location('builder38',tools/'apply-fix331038.py');builder=importlib.util.module_from_spec(spec);spec.loader.exec_module(builder)
    ...
    check('version',mf['version']=='1.14.0.43' and mf['version_name']=='1.14.0 ShunCode MCP Fix 3.3.10.38')
    ...
    for lang in ['en','zh_CN']:
        d=json.loads((root/f'_locales/{lang}/messages.json').read_text(encoding='utf-8'));check('locale:'+lang,d['extension_name']['message']=='DeepSeek++ ShunCode MCP Fix 3.3.10.38')
    suites=sorted(root.glob('*selftest*.js'));check('55_legacy_suites',len(suites)==55)
    for n in ['apply-fix331037.py','apply-fix331038.py','validate-fix331038.py']:
    ...
    report={'release':'v1.14.0-fix3.3.10.38','passed':all(x['passed'] for x in checks+tests),'checks':checks,'tests':tests,'runtime_sha256':hashes,'browser_model_acceptance':'pending: reload extension, run read_image in a tab that switched conversation without F5; expect upload_ok -> refs -> ack and a real visual answer; not proven by offline fixtures'}
