...
& python -c "import json,pathlib,sys; r=pathlib.Path(sys.argv[1]); files=['manifest.json','_locales/en/messages.json','_locales/zh_CN/messages.json']; objs=[json.loads((r/f).read_text(encoding='utf-8-sig')) for f in files]; assert objs[0].get('version_name')=='1.14.0 ShunCode MCP Fix 3.3.10.11'; assert objs[0].get('version')=='1.14.0.16'" $Root
...
$markers=@(
 ...
 @('fix331010 resume history injection',$content,'toolExecutions:DPPResume?.toolExecutions??n'),
 ...
 @('fix331011 manual safe wiring',$content,'DPPManualResume331010=DPP_MANUAL_RESUME_PROMPT_SAFE_331011'),
 @('fix331011 agent safe wiring',$content,'DPPResume=DPP_AGENT_RESUME_PREPARE_331011'),
