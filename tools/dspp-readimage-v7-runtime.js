/* DPP_READIMAGE_V7_BEGIN - Web-only, run-scoped image handoff. */
function DPP_CREATE_READIMAGE_V7(options) {
  ...
  function diag(stage, fields = {}) {
    const row = {version: 7, time: Date.now(), stage, ...fields};
    try {
      const key = 'dpp_read_image_diag_v7';
      let rows;
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        chrome.runtime.sendMessage({type: 'UPLOAD_DEEPSEEK_IMAGE', payload: {
          ...
          if (runtimeError) return finish(new Error('runtime_unavailable'));
          if (response?.ok !== true) {
            const code = response?.code || response?.error;
            const safe = new Set(['runtime_message_unauthorized', 'runtime_message_invalid', 'chat_disabled', 'upload_busy', 'auth_missing', 'auth_rejected', 'auth_failed', 'pow_failed', 'invalid_image', 'upload_aborted', 'upload_failed']);
            return finish(new Error(safe.has(code) ? code : 'upload_failed'));
          }
          if (typeof id !== 'string' || !id.trim() || id.length > 256) return finish(new Error('invalid_upload_response'));
          finish(null, id);
        });
      } catch { finish(new Error('runtime_unavailable')); }
    });
  }
  async function capture(execution) {
    ...
    for (const image of images) {
      ...
      try {
        ...
        const codes = new Set(['upload_timeout', 'runtime_unavailable', 'runtime_message_unauthorized', 'runtime_message_invalid', 'chat_disabled', 'upload_busy', 'auth_missing', 'auth_rejected', 'auth_failed', 'pow_failed', 'invalid_image', 'upload_aborted', 'invalid_upload_response', 'upload_failed']);
        note(codes.has(error?.message) ? error.message : 'upload_failed');
      }
    }
    return cleaned;
  }
