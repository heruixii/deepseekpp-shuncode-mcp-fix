...
try{
  ...
  $currentOverlaySource=$sourceVersion -in @('1.14.0 ShunCode MCP Fix 3.3.2','1.14.0 ShunCode MCP Fix 3.3.3','1.14.0 ShunCode MCP Fix 3.3.4','1.14.0 ShunCode MCP Fix 3.3.5','1.14.0 ShunCode MCP Fix 3.3.6','1.14.0 ShunCode MCP Fix 3.3.7')
  ...
  if($sourceVersion -notin @('1.14.0 ShunCode MCP Fix 3.3.6','1.14.0 ShunCode MCP Fix 3.3.7')){
    & python $Apply336 $dst
    if($LASTEXITCODE -ne 0){throw "apply-fix336.py failed with exit $LASTEXITCODE"}
  }
  if($sourceVersion -ne '1.14.0 ShunCode MCP Fix 3.3.7'){
    & python $Apply337 $dst
    if($LASTEXITCODE -ne 0){throw "apply-fix337.py failed with exit $LASTEXITCODE"}
  }else{Write-Host 'BUILD_FIX3 Fix 3.3.7 source detected; current overlay skipped'}
  ...
  foreach($name in @('apply-fix3.py','apply-fix31.py','apply-fix32.py','apply-fix33.py','apply-fix331.py','apply-fix332.py','apply-fix333.py','apply-fix334.py','apply-fix335.py','apply-fix336.py','apply-fix337.py','health-check.ps1','build-fix3.ps1')){
