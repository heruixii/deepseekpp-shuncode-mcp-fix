...
const source = full ? full.slice(full.indexOf('/* DPP_READIMAGE_V7_BEGIN'), full.indexOf('/* DPP_READIMAGE_V7_END */') + '/* DPP_READIMAGE_V7_END */'.length) : fs.readFileSync(path.join(__dirname, 'dspp-readimage-v7-runtime.js'), 'utf8');
...
function setup(send, fastTimeout = false) {
  ...
  const state = box.DPP_CREATE_READIMAGE_V7({backend: 'web', signal: controller.signal, chatSessionId: 'session-a', onDiagnostic: row => rows.push(row)});
...
(async () => {
  ...
  await test('API backend unchanged and no upload', async () => {
    const x = setup(); const api = x.box.DPP_CREATE_READIMAGE_V7({backend: 'official-api'}); const e = execution();
  ...
  await test('server failure, invalid ID and runtime.lastError reported', async () => {
    for (const response of [{ok: false, error: 'secret-token'}, {ok: true, file: {}}, {ok: true, file: {id: 7}}]) {
      ...
      const r = (await request(x.state)).value; assert.equal(r.refFileIds.length, 0); assert(r.prompt.includes(response.ok ? 'invalid_upload_response' : 'upload_failed'));
  ...
  await test('diagnostic ring <=64, no pixels, full path, auth or file IDs', async () => {
    ...
    const text = x.stored.get('dpp_read_image_diag_v7'); assert(JSON.parse(text).length <= 64);
  ...
  if (full) {
    await test('candidate has no v5 global queue; native forwarding preserved', async () => {
      ...
      assert(full.includes('finally{DPPVisionV6.close()}')); // owning-loop variable is intentionally retained
    });
