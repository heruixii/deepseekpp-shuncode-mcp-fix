'use strict';
// .37 upload-gate diagnostics: actual extracted listener/gates/service from the candidate background.js.
...
if (!root || !baselineRoot) throw Error('Usage: node dspp-upload-gate-diag-v9-selftest.js candidate-root baseline-36-root');
...
(async () => {
  const CASES = [
    ...
    ['tab has conversation, sender url does not', {...sender(), url: 'https://chat.deepseek.com/'}, 'ctx_sender_session_missing'],
    ['sender and tab conversations differ', {...sender(), url: URL_B}, 'ctx_sender_tab_session_mismatch'],
  ...
  await test('every synchronous rejection keeps its verdict and gains a distinct fixed reason', async () => {
    for (const [name, from, reason] of CASES) {
      ...
      if (reason === null) { assert.equal(r.ok, true, name); continue; }
  ...
  await test('asynchronous tab rejections (aI/TN) carry reasons', async () => {
    ...
    h = harness(); h.state.tab = {id: 7, url: URL_B}; denied(await h.dispatch({type: 'UPLOAD_DEEPSEEK_IMAGE', payload: payload()}, {...sender(), tab: {id: 7, url: URL_B}}), 'ctx_sender_tab_session_mismatch'); await noUploadSideEffects(h);
    // tabs.get already shows another conversation: TN rewrites chatSessionId, REQUIRE catches the sender/tab mismatch before ASSERT.
    h = harness(); h.state.tab = {id: 7, url: URL_B}; denied(await h.dispatch({type: 'UPLOAD_DEEPSEEK_IMAGE', payload: payload()}), 'ctx_sender_tab_session_mismatch'); await noUploadSideEffects(h);
  });
  ...
  await test('probe reports booleans matching the field-observed shape', async () => {
    ...
    const r2 = await harness().dispatch({type: 'UPLOAD_DEEPSEEK_IMAGE', payload: payload()}, {...sender(), url: URL_B});
    assert.equal(r2.probe.sameSession, false); assert.equal(r2.probe.ctxSameSession, false); assert.equal(r2.probe.ctxTabSession, true);
  });
  ...
  await test('gate source untouched: wN/TN/EN/aI/REQUIRE/ASSERT/DISPATCH bodies identical to .36 except diag wrapping', async () => {
    for (const [a, b] of [['function wN(', 'function TN('], ['function TN(', 'function EN('], ['function EN(', 'function DN('], ['async function aI(', 'async function oI('],
      ['function DPP_REQUIRE_UPLOAD_CONTEXT_V7(', 'async function DPP_ASSERT_CURRENT_UPLOAD_V7('], ['async function DPP_ASSERT_CURRENT_UPLOAD_V7(', 'async function DPP_DISPATCH_UPLOAD_V7(']]) {
  ...
  await test('content diag: gate_reason row with whitelisted fields, blocker note, no retry loop', async () => {
    for (const [from, reason] of [[{...sender(), documentLifecycle: 'prerender'}, 'sender_lifecycle_not_active'], [{...sender(), url: URL_B}, 'ctx_sender_tab_session_mismatch']]) {
  ...
  console.log(`RESULT ${groups}/${groups} upload-gate diagnostic groups passed; actual extracted code, offline dependencies only.`);
