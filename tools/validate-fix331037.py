#!/usr/bin/env python3
"""Offline .37 release gate; never modifies live installs or uploads images.
python tools/validate-fix331037.py --root CANDIDATE --baseline BASE_36 --output NEW_EXTERNAL_DIR
.37 intentionally changes background.js (diagnostics only). Allowed-change list replaces the .36 'background unchanged' lock.
"""
import argparse
import ast
import hashlib
...
def main():
    ...
    spec=importlib.util.spec_from_file_location('builder37',tools/'apply-fix331037.py');builder=importlib.util.module_from_spec(spec);spec.loader.exec_module(builder)
    rebuilt=builder.build(base,out/'rebuilt')
    inventory=json.loads((tools/'fix331037-baseline-sha256.json').read_text(encoding='utf-8'))
    allowed={'background.js','content-scripts/content.js','manifest.json','_locales/en/messages.json','_locales/zh_CN/messages.json'}
    ...
    check('gate_functions_unchanged',all(seg(bg,x,y)==seg(old,x,y) for x,y in [('function wN(','function TN('),('function TN(','function EN('),('function EN(','function DN('),('function DN(','function ON('),('async function aI(','async function oI('),('function DPP_REQUIRE_UPLOAD_CONTEXT_V7(','async function DPP_ASSERT_CURRENT_UPLOAD_V7('),('async function DPP_ASSERT_CURRENT_UPLOAD_V7(','async function DPP_DISPATCH_UPLOAD_V7(')]))
    ...
    check('version',mf['version']=='1.14.0.42' and mf['version_name']=='1.14.0 ShunCode MCP Fix 3.3.10.37')
    ...
    for lang in ['en','zh_CN']:
        d=json.loads((root/f'_locales/{lang}/messages.json').read_text(encoding='utf-8'));check('locale:'+lang,d['extension_name']['message']=='DeepSeek++ ShunCode MCP Fix 3.3.10.37')
    suites=sorted(root.glob('*selftest*.js'));check('55_legacy_suites',len(suites)==55)
    for n in ['apply-fix331037.py','validate-fix331037.py']:
    ...
    run('upload-gate-diag-v9',['node',str(tools/'dspp-upload-gate-diag-v9-selftest.js'),str(root),str(base)])
    ...
    report={'release':'v1.14.0-fix3.3.10.37','passed':all(x['passed'] for x in checks+tests),'checks':checks,'tests':tests,'runtime_sha256':hashes,'browser_model_acceptance':'pending: reload, reproduce rejection, read dpp_upload_gate_diag_v9 / gate_reason rows; not proven by offline fixtures'}
