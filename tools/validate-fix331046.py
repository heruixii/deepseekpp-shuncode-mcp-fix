#!/usr/bin/env python3
"""Independent .46 reconstruction, exact runtime delta, permissions invariance and executable suites."""
...
def main():
    ...
    b46=load('46');rebuilt=b46.build(base,out/'rebuilt');b45=load('45');old=b45.build(base,out/'base45')
    ...
    check('runtime_delta_only_background_manifest_locales',runtime_changes=={'background.js','manifest.json','_locales/en/messages.json','_locales/zh_CN/messages.json'})
    expected=old['background.js']
    for before,file in [(b45.NEW_NU,'dspp-cache-nu-v46.js'),(b45.NEW_SC,'dspp-cache-sc-v46.js')]:expected=b45.once(expected,before.encode(),(tools/file).read_text('utf-8').strip().encode())
    check('background_exactly_Nu_Sc_only',actual['background.js']==expected)
    for n in ['content-scripts/content.js','content-scripts/main-world.js','fix3-policy.js']:check('unchanged:'+n,actual[n]==old[n])
    ...
    check('manifest_version',m['version']=='1.14.0.51' and m['version_name'].endswith('Fix 3.3.10.46'))
    ...
    for lang in ['en','zh_CN']:check('locale:'+lang,json.loads(actual[f'_locales/{lang}/messages.json'])['extension_name']['message']=='DeepSeek++ ShunCode MCP Fix 3.3.10.46')
    def run(n,cmd):
    ...
    suites=sorted(root.glob('*selftest*.js'));check('62_suites',len(suites)==62)
    ...
    for name,args in [('dspp-skill-popup-observe-selftest.js',[root,base]),('dspp-mw-bridge-diag-v10-selftest.js',[root,base]),('dspp-upload-gate-v10-selftest.js',[root,base]),('dspp-visual-workflow-v8-selftest.js',[root]),('dspp-readimage-v7-selftest.js',[root/'content-scripts/content.js']),('dspp-readimage-v7-boundary-selftest.js',[root])]:run('external-'+name,['node',str(tools/name),*map(str,args)])
    report={'release':'v1.14.0-fix3.3.10.46','passed':all(x['passed'] for x in checks+tests),'checks':checks,'tests':tests,'runtime_sha256':{n:sha(actual[n]) for n in sorted(runtime_changes)},'suites':len(suites)}
