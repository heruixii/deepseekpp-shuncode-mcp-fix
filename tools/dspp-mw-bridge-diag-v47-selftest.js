...
(async () => {
  await test('helper present once, baseline untouched elsewhere', () => {
    ...
    // .47 deltas are separately executed by startup-ui-v47; reverse EXACT deltas here.
    for(const [before,after] of [
      ['expiresAt:Date.now()+12e3','expiresAt:Date.now()+10*60e3'],
      ['function to(e,t){let n=!1','function to(e,t){const DPPRequestMeta331047=t;let n=!1'],
      ['DPP_TRACK_GENERATION_RESULT_331020(t,a)','DPP_TRACK_GENERATION_RESULT_331020(DPPRequestMeta331047,a)']
    ]){assert.equal(back.split(after).length,2,'exact approved .47 delta: '+after);back=back.replace(after,before);}
