...
test('Agent launch reads durable traces before starting', src.includes('DPP_AGENT_RESUME_PREPARE_331012(e,await k0())'));
test('raw user prompt is persisted into the new trace', src.includes('BY=b0(e,r,t,p,s)'));
test('historical tool objects are not replayed into the continuation loop', src.includes('toolExecutions:n') && !src.includes('toolExecutions:DPPResume?.toolExecutions??n'));
const requestAugmenter = between('function Zc(e,t)', 'function Qc(e)');
test('manual request prompt remains the raw user input', !requestAugmenter.includes('DPP_MANUAL_RESUME_PROMPT_SAFE_331011'));
