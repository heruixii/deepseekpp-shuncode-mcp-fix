...
const base={version:1,adaptiveMaxDirectTools:5,adaptiveMaxPromptBytes:14000,servers:{srv:{mode:'direct',pinnedDescriptorIds:['keep-me']}}};
let small=[d('read_files','mcp','srv',100)];let a=p.promptExposureSettings(small,base,'读取文件');
ok('small-keeps-settings',a===base);
let big=Array.from({length:12},(_,i)=>d(`tool_${i}`,'mcp','srv',2400));let b=p.promptExposureSettings(big,base,'读取文件');
...
ok('original-not-mutated',base.servers.srv.mode==='direct');
...
if(failed)process.exit(1);console.log('ALL_PASS 12');
